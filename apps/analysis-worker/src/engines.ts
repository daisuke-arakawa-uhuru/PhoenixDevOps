...
export interface SsotSynthesisGenerator {
  generate(
    payload: AnalysisTaskPayload,
    sourceSpecification: ExtractionResult,
    documentSpecification: ExtractionResult,
...
export class GeminiSsotSynthesisGenerator implements SsotSynthesisGenerator {
  constructor(private geminiClient: GeminiClient) {}
...
export class PlaceholderSsotSynthesisGenerator implements SsotSynthesisGenerator {
  async generate(
    payload: AnalysisTaskPayload,
    sourceSpecification: ExtractionResult,
    documentSpecification: ExtractionResult,
    ...
    return [
      ...
      "",
      "This is a placeholder artifact generated by the analysis worker skeleton.",
      "It must not be treated as the final implementation-derived system specification.",
      "",
      "## Analysis Target",
      "",
      `- Project: ${title}`,
      `- Job ID: ${payload.jobId}`,
      `- Source summary length: ${sourceSpecification.summary.length}`,
      `- Document summary length: ${documentSpecification.summary.length}`,
      `- Infrastructure spec: ${componentSpecifications.infrastructureSpecMarkdown ? "provided" : "missing"}`,
      `- API spec: ${componentSpecifications.apiSpecMarkdown ? "provided" : "missing"}`,
      `- Database spec: ${componentSpecifications.databaseSchemaSpecMarkdown ? "provided" : "missing"}`,
      `- Business logic spec: ${componentSpecifications.businessLogicSpecMarkdown ? "provided" : "missing"}`,
      "",
      "## Pending Implementation",
      "",
      "- SSOT synthesis with Gemini API",
      "- Mermaid system component diagram",
      "- Endpoint-to-DB data flow description",
      "",
    ].join("\n");
  }
}
...
export function generatedArtifacts(
  ...
  return {
    ...
    asFiles() {
      ...
      if (this.ssotMarkdown) {
        files["single-source-of-truth.md"] = this.ssotMarkdown;
      }
