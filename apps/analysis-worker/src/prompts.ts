...
export function buildApiSpecificationAnalysisPrompt(
  payload: AnalysisTaskPayload,
  inputs: AnalysisInput,
  apiRelatedFiles: Array<{ path: string; content: string }>,
  codebaseMapJson: string,
  exportedSymbolsSummary: string,
): string {
  return [
    "[TASK: API_SPECIFICATION_ANALYSIS]",
    "",
    "あなたはレガシー Web アプリケーションの API・インターフェースを解析する専門エンジンです。",
    "ルーティング定義、コントローラー（Controller）クラス、ハンドラー、ミドルウェア、バリデーションロジックの",
    "ソースコードを読み取り、全APIエンドポイント、パラメータ、レスポンス構造などを抽出し、",
    "OpenAPI/Swagger に準拠した形式で「API仕様書」を生成します。",
    "",
    "## 解析対象",
    "",
    `- ジョブID: ${payload.jobId}`,
    ...
    "",
    "- ソースコードを正として扱い、根拠を示せない内容は断定しないでください。",
    "- 各エンドポイントについて、URL、HTTPメソッド、リクエストパラメータ（パス/クエリ/ボディ）、",
    "  レスポンス形式、ステータスコード、認証要件を可能な限り詳細に抽出してください。",
    "- バリデーションルール（必須/任意、型、最小値/最大値、パターン等）はソースコードから忠実に抽出してください。",
    "- ミドルウェアやインターセプターによる共通処理（認証、ログ、CORS等）も把握してください。",
    "- 解析できない箇所は「判断不能」として明示してください。",
    "- ORM定義やDB直接アクセスから推測したレスポンス構造は「推測」と明示してください。",
    "",
    "## 出力形式（必ず以下のMarkdown構成で出力してください）",
    "",
    "### 出力章",
    "",
    "1. **解析対象サマリー** — 解析したAPI関連ファイル数、検出したエンドポイント数、使用フレームワーク",
    "2. **API概要** — ベースURL/パス、APIバージョニング方式、共通ヘッダー/認証方式の概要",
    "3. **エンドポイント一覧** — 全エンドポイントのテーブル形式:",
    "   - HTTPメソッド",
    "   - パス",
    "   - 処理概要",
    "   - コントローラー/ハンドラー名",
    "   - 認証要否",
    "   - 根拠ファイルパス",
    "4. **エンドポイント詳細** — 各エンドポイントについて:",
    "   - エンドポイント名（`METHOD /path` 形式）",
    "   - 処理概要",
    "   - パスパラメータ（名前、型、必須/任意、説明）",
    "   - クエリパラメータ（名前、型、必須/任意、デフォルト値、説明）",
    "   - リクエストボディ（Content-Type、スキーマ、各フィールドの型・制約・バリデーション）",
    "   - レスポンス（ステータスコード、Content-Type、レスポンスボディスキーマ）",
    "   - エラーレスポンス（想定されるエラーコードとメッセージ）",
    "   - 根拠ファイルパスと行番号",
    "5. **リクエスト/レスポンス共通スキーマ** — 再利用される型定義・DTO・モデル",
    "   - スキーマ名、フィールド一覧（名前、型、必須/任意、説明）をテーブル形式で出力",
    "6. **認証・認可** — 認証方式（JWT、API Key、OAuth等）、認可（ロール、権限）の詳細",
    "7. **ミドルウェア・共通処理** — 全エンドポイントまたは特定グループに適用される共通処理",
    "8. **エラーハンドリング** — 共通エラーレスポンス形式、エラーコード体系",
    "9. **判断不能・推測事項** — 解析できなかった箇所とその理由",
    "",
    "## STEP 1 成果物: コードベースマップ（codebase-map.json）",
    "",
    "```json",
    ...
    "## API関連ソースコード",
    "",
    formatTextFiles(apiRelatedFiles),
  ].join("\n");
}
