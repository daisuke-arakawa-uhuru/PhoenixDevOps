...
- `src/infra.js`: インフラ・IaC個別解析エージェント（Issue #31）。Terraform / docker-compose / Kubernetes を静的抽出し Gemini で `infrastructure_spec.md` を生成
...
| `infrastructure_spec.md` | Terraform / docker-compose / Kubernetes から逆算したインフラ物理/論理構成・セキュリティ設計（Issue #31） |
...
現時点では Gemini prompt と呼び出し口、入力本文取得、軽量な事前構造解析までの実装です。インフラ・IaC個別解析エージェント（`src/infra.js`）は外部 CLI に依存せず、Terraform の provider/resource/data/module/variable/output/backend、docker-compose のサービス、Kubernetes マニフェストを静的抽出し、セキュリティグループ/ファイアウォール・IAM 等のセキュリティ関連リソースを着目点として整理した上で Gemini に渡します。PDF は `pdf-parse`、Excel は xlsx 内 XML の軽量抽出、ZIP は標準ライブラリベースの読み取りで扱います。
