...
`succeeded` のジョブだけ成果物の署名付き URL を返します。レスポンスは Firestore の `artifact_paths` に保存された全成果物を返すため、`single-source-of-truth.md`、`true-design.md`、`document-drift-report.md`、`database_schema_spec.md` などの追加成果物も同じ形式で取得できます。
